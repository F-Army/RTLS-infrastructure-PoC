...
describe("/anchor/ Anchor route tests", () => {
    ...
    it("should send back 400 if receives invalid data", async () => {
        ...
        const res = await request(app).post("/anchor/1").send("test=Invalid&test2=Invalid");
    ...
    it("should send 200 if data is valid", async () => {
        ...
        const short = 0x01;
        const app = initRoute(anchorRoute);
        const res = await request(app).post(`/anchor/${eui}`).send(`short=${short}&x=0&y=0&z=0`);
    ...
    it("should return 409 if trying to save the same anchor (eui)", async () => {
        ...
        const newShort = 0x02;
        const app = initRoute(anchorRoute);
        await request(app).post(`/anchor/${eui}`).send(`short=${oldShort}&x=-0.1&y=0.1&z=0.1`);
        const res2 = await request(app).post(`/anchor/${eui}`).send(`short=${newShort}&x=-0.1&y=0.1&z=0.1`);
